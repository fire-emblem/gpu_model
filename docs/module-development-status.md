...
| `M6` | Functional 执行核心 | 单线程和多线程共用一套 functional core；支持 wave/block/device 层级执行；支持 `st/mt` 切换 | `Partial` | 已新增显式 wave run state，`FunctionalExecEngine` 的 block barrier wait/resume 已收敛到单一恢复点，`st/mt` shared-barrier 回归已锁住一致性 | 还缺更完整 wait reason 扩展；还缺对任意 HIP 程序的大规模稳定性验证 |
