...
| `M6` | Functional 执行核心 | 单线程和多线程共用一套 functional core；支持 wave/block/device 层级执行；支持 `st/mt` 切换 | `Partial` | 已有共享 `FunctionalExecutionCore`；`st/mt` 已共核；已有 PEU-local wave pool、round-robin、block 内 shared/barrier kernel 的 `mt` 路径；marl 已接入；`1D/2D/3D` launch 配置、placement 和 canonical `xyz` builtin 主线已打通 | 还缺更完整 wait/resume 抽象；还缺对任意 HIP 程序的大规模稳定性验证；还缺与 raw GCN path 的共享执行部件收拢 |
