...
TEST(OpPlanApplyTest, AppliesRegisterWritesAndFormatsExecUpdate) {
  WaveState wave;
  ...
  const auto mask_text = MaybeFormatExecMaskUpdate(plan, wave);
  ASSERT_TRUE(mask_text.has_value());
  EXPECT_FALSE(mask_text->empty());
}

TEST(OpPlanApplyTest, AppliesControlFlowAndExitSemantics) {
  ...
  ApplyPlanControlFlow(branch_plan, branch_wave, true, true);
  ...
  ApplyPlanControlFlow(exit_plan, exit_wave, true, true);
