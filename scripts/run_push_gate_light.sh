...
JOBS="${JOBS:-8}"
SMOKE_FILTER="${GPU_MODEL_GATE_LIGHT_GTEST_FILTER:-RuntimeNamingTest.*:RuntimeProgramCompatibilityAliasTest.*:HipInterposerStateTest.RunsHipHostExecutableThroughLdPreloadInterposer:HipRuntimeTest.LaunchKernelCanReadMaterializedDataPool:ModelRuntimeCoreTest.SimulatesMallocMemcpyLaunchAndSynchronizeFlow:TraceTest.NativePerfettoProtoContainsHierarchicalTracksAndEvents}"
...
run_release_smoke() {
  ...
  cmake --build "$RELEASE_BUILD_DIR" --target gpu_model_tests gpu_model_hip_interposer -j "$JOBS" \
...
run_debug_asan_smoke() {
  ...
  cmake --build "$DEBUG_BUILD_DIR" --target gpu_model_tests gpu_model_hip_interposer -j "$JOBS" \
