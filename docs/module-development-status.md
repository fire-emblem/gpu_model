...
- 全量 `gpu_model_tests` 当前结果为 `488 passed`
...
| `M13` | Cycle model | 完整 cycle 建模、issue/latency/waitcnt/event/timeline | `Partial` | 已有 naive cycle 主干、issue model、waitcnt 领域阻塞、event queue、timeline、Google trace、cache/bank conflict/waitcnt cycle 测试；barrier wait/release 状态机已与 shared/barrier cycle kernel、block reduction、softmax reduction、transpose 等回归重新对齐；functional `st/mt` 现已新增与 cycle mode 结果形状兼容的 executed-flow program cycle 统计，可用于整程序理论值对照与后续校准，且已覆盖大 block / 多 wave / 多 block 的 mixed-cost 与非对称 workload 回归；`global/shared/scalar-buffer` 不同 memory-arrive + waitcnt，以及 single/double barrier 的程序统计增长也已由 focused regression 锁定；代表性 case 的排序、分类统计求和、自 `st/mt` 模式一致性已由 focused regression 锁定，可用于 coarse program-level cycle evaluation | 仍缺更完整的架构资源冲突、更多 memory domain/pipe 细节、与真实硬件差异说明和参数化建模文档 |
