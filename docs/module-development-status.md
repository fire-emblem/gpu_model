...
| `M7` | 内存系统与地址空间 | global/shared/private/constant/kernarg/data/managed 独立地址空间，host/device 拷贝与 map 映射 | `Partial` | 已有多 memory pool、managed、kernarg、constant、device load materialize、host/device 基本 memcpy、fake device ptr 到 model addr 映射 | 还缺统一的 `DeviceMemoryManager`；还缺 compatibility virtual windows；还缺 `reserve big range + commit on demand` 主线；还缺不同 memcpy 方向与同步语义的轻量测试矩阵；还缺 data/const/bss/relocation 更完整装载；还缺 map/unmap 语义完善 |
