...
├── dpc/dpc_def.h            # DPC 定义
├── device/device_def.h      # Device 定义
├── issue_config/            # issue policy types
├── memory/                  # Memory 定义 (memory_pool, memory_space, cache_model, etc.)
├── peu/peu_def.h            # PEU 定义
...
│   └── cycle_exec_engine.h
├── encoded/                 # 编码执行引擎
│   ├── encoded_semantic_handler.h
│   └── program_object_exec_engine.h
├── functional/              # 功能执行引擎
│   ├── functional_exec_engine.h
│   └── functional_execution_mode.h
└── internal/                # 共享调度逻辑
    ├── cycle_types.h
    ├── cycle_wave_schedule.h
    ├── cycle_issue_schedule.h
    └── ...
```
...
│   └── ap_runtime_state.h
├── device/                  # Device 运行时状态
│   └── device_runtime_state.h
├── dpc/                     # DPC 运行时状态
│   └── dpc_runtime_state.h
├── memory/                  # Memory 运行时状态桥接
│   └── memory_runtime_state.h
├── peu/                     # PEU 运行时状态
│   └── peu_runtime_state.h
├── register/                # Register 运行时状态桥接
│   └── register_runtime_state.h
└── wave/                    # Wave 运行时状态
    └── wave_runtime_state.h
```
