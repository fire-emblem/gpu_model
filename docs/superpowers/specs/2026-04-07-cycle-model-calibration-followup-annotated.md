...
- AC-5: front-end / dispatch / slot / switch 相关 cycle 事件在 engine 中成为真实状态边，并通过统一 typed event 对外暴露
  - Positive Tests (expected to PASS):
    - `block_admit`、`wave_generate`、`wave_dispatch`、`slot_bind`、`issue_select`、`wave_switch_away`、`wave_exit` 等事件在 cycle engine 中有清晰来源和时间点
...
1. Milestone 1: execution semantics calibration
   ...
   - Phase B: 把真实状态边全部落到 execution/state-machine
   - Phase C: 用 focused regressions 固化 `arrive_still_blocked / arrive_resume / switch away / resume`

2. Milestone 2: recorder protocol unification
   - Phase A: 审计 st / mt / cycle 当前 recorder 生产路径
...
|---------|-------------|-----------|----------------------------|------------|
| task1 | 审计 `waitcnt / arrive / barrier / switch away / resume` 的当前 execution 语义，标记仍停留在 consumer 层的残余逻辑 | AC-1, AC-5 | analyze | - |
| task2 | 基于审计结果校准 functional / cycle execution 的真实状态边与 typed event 产生点 | AC-1, AC-5 | coding | task1 |
| task3 | 为 waitcnt-heavy、barrier-heavy、switch/resume 相关语义补 focused regressions | AC-1, AC-3, AC-5 | coding | task2 |
| task4 | 审计 st / mt / cycle 当前 recorder 生产路径及 debug 模块边界，列出分裂点与跨模块依赖问题 | AC-2, AC-6 | analyze | task2 |
| task5 | 统一 recorder 协议、公开头文件接口与 cycle range 源头记录规则 | AC-2, AC-3, AC-6 | coding | task4 |
