...
- 全量 `gpu_model_tests` 当前结果为 `826 passed`
- **架构重构 Phase 1-5 已完成**：
...
- **分层违规修复状态**：
  - V1/V2/V3/V4/V5: ✅ 全部已修复
  - execution→runtime 依赖已消除（共享类型移至 utils/config/ 和 execution/stats/）
- **最终态收尾**：
  - `src/` 活跃结构中的剩余 trivial bridge headers 已删除
  - `./scripts/run_push_gate.sh` 已在 final sweep 后重新通过
  - `runtime/model_runtime/stats/` 空目录已清理
...
- `cycle issue` 的正式 `eligible -> selected -> issue` 语义、参考资料中的 `round_robin / oldest_first` 策略，以及当前工程里的 `ArchitecturalIssuePolicy / IssueScheduler` 映射，现统一记录在 [cycle-issue-eligibility-policy.md](./cycle-issue-eligibility-policy.md)
