...
- AC-1: `InstructionIssue -> Commit` 配对只为应显示的指令生成 slice，且 `issue_cycle / commit_cycle / render_duration_cycles` 与执行事实一致
  - Positive Tests (expected to PASS):
    - `tests/runtime/cycle_timeline_test.cpp` 中 issue/commit 顺序回归通过，`buffer_load_dword`、普通 ALU 指令可稳定生成 slice
    - `tests/runtime/trace_test.cpp` 中 encoded functional instruction slice 仍保持 `dur=4`
...
- AC-2: `arrive` 事件的 cycle、命名和可视表达稳定，且 `still_blocked / resume` 语义不混淆
  - Positive Tests (expected to PASS):
    ...
    - `cycle_timeline_test.cpp` 与 `trace_test.cpp` 中 `arrive` 出现在正确时间顺序上
...
- AC-6: 文档同步收口，明确 cycle timeline 的正式语义与限制
  - Positive Tests (expected to PASS):
    - `docs/my_design.md`、`docs/module-development-status.md`、必要时 `task_plan.md` 回写本阶段结论
...
- `cycle_timeline.cpp` 和相关 trace exporter 规则清晰
...
- Can use:
  ...
  - 增加 focused tests
  - 调整 `cycle_timeline.cpp` 的 slice/marker 生成规则
...
> **Note on Deterministic Designs**: 本计划的关键设计约束较窄。trace/timeline 只能消费事件、不能承担业务推断，因此上界和下界都围绕“消费层校准”展开，而不是允许多条完全不同的实现路线。
...
2. 把当前测试分成：
   ...
   - 意图错误或过度绑定实现
   - 能直接充当校准基线
3. 修 `cycle_timeline.cpp` 时只做消费层收口：
   - issue/commit 配对
   - marker 分类
   - duration 来源
...
- [docs/cycle-timeline-accuracy-calibration-draft.md](/data/gpu_model/docs/cycle-timeline-accuracy-calibration-draft.md) - 本计划的原始草稿
...
2. Milestone 2: focused tests 审计与重写
   - Phase A: 审 `tests/runtime/cycle_timeline_test.cpp`
   - Phase B: 审 `tests/runtime/trace_test.cpp` 中 timeline 相关用例
...
| task2 | 审计 `tests/runtime/cycle_timeline_test.cpp` 现有断言，标记保留/重写/删除项 | AC-1, AC-2, AC-3 | analyze | task1 |
| task3 | 审计 `tests/runtime/trace_test.cpp` 中与 timeline 相关的断言，标记保留/重写/删除项 | AC-1, AC-2, AC-3, AC-4 | analyze | task1 |
| task4 | 根据真值表重写 focused tests，形成稳定的顺序与命名回归 | AC-1, AC-2, AC-3, AC-4 | coding | task2, task3 |
| task5 | 校准 `src/debug/timeline/cycle_timeline.cpp` 中 issue/commit 配对和 marker 分类规则 | AC-1, AC-2, AC-3, AC-4 | coding | task4 |
...
- DEC-1: examples 校准的覆盖范围
  - Claude Position: 先做最小代表集，至少 1 组 waitcnt-heavy 和 1 组 barrier-heavy
  - Codex Position: 同意，先用代表集形成闭环，再决定是否全量扩展
  - Tradeoff Summary: 先小范围代表集能更快闭环；直接全量 examples 成本高且噪声大
