...
- `HipRuntime compatibility naming cleanup` 作为并行整理项持续存在，但不应阻塞 correctness 主线。
- 其目标是移除“interposer 是独立模块”的历史含义，而不是移除 `LD_PRELOAD` 入口本身。
...
4. `HipRuntime compatibility naming cleanup`
   - 语义收口、测试命名、target 命名、日志命名
   - 可并行推进，但避免与 `include -> src` 物理迁移同时大规模重命名相同文件
