...
| `M1` | Host runtime 与 HIP runtime 基础 | 单卡、单 context、单 stream、同步 runtime 入口、基本错误码与设备选择 | `Partial` | 已有 `HostRuntime`、`RuntimeHooks`、`ModelRuntimeApi` facade、`hipMalloc/hipFree/hipMemcpy/hipLaunchKernel/hipGetDeviceCount/hipGetDevice/hipSetDevice/hipGetDeviceProperties/hipDeviceGetAttribute`、基础 stream/event 空实现，以及 model-native 统一 `LoadModule` 入口 | 还缺 property / attribute 更完整覆盖；还缺 context/stream 边界文档化与统一限制；还缺更完整同步 runtime 子集梳理 |
