...
ExecutableKernel BuildSharedTransposeKernel() {
  ...
  builder.SMul("s6", "s2", "s4");  // input width
  builder.SMul("s7", "s3", "s5");  // input height

  builder.VMul("v4", "v1", "s6");  // input row offset
  builder.VAdd("v5", "v4", "v0");  // input linear index

  builder.VMul("v6", "v3", "s2");  // local shared row offset
  ...
  builder.BExit();
  return builder.Build("shared_transpose_2d");
}
...
TEST(TransposeSharedFunctionalTest, TransposesRectangularTileUsingSharedMemory) {
  ...
  for (uint32_t y = 0; y < height; ++y) {
    for (uint32_t x = 0; x < width; ++x) {
      ...
      runtime.memory().StoreGlobalValue<int32_t>(out_addr + index * sizeof(int32_t), -1);
    }
  }
  ...
  EXPECT_GE(result.stats.shared_loads, 2u);
